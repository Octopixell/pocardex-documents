# END USER LICENSE AGREEMENT FOR POCARDEX

This End User License Agreement ("EULA") is a legal agreement between Hyperdrive Labs, located in The Netherlands ("Licensor"), and you ("User") for the Pocardex mobile application ("App"). By downloading, installing, or using the App, you accept these terms. If you do not agree, do not use the App.

## 1. Grant of License

Licensor grants User a limited, non-exclusive, non-transferable, revocable license to use the App on one compatible device for personal, non-commercial purposes only.

## 2. Restrictions

User shall not:  
(a) copy, modify, reverse engineer, decompile, or disassemble the App;  
(b) distribute, sublicense, or commercially exploit the App or Pokémon card data;  
(c) remove copyright notices;  
(d) use the App illegally or to harm others;  
(e) bypass security features like authentication.

## 3. Intellectual Property

All rights, title, and interest in the App, including code and dependencies (e.g., data storage, sign-in), remain with Licensor. User gains no ownership rights.

## 3.1 Pokémon Disclaimer

Pocardex is an unofficial fan application not affiliated with, endorsed by, sponsored by, or supported by Nintendo, GAME FREAK, Creatures Inc., or The Pokémon Company (collectively, "Pokémon Owners"). All Pokémon names, images, artwork, characters, and related content displayed in the App are trademarks and copyrights owned by the Pokémon Owners and used under fair use guidelines for non-commercial, informational, and transformative purposes only. No copyright or trademark infringement is intended. User acknowledges that all rights to Pokémon intellectual property belong exclusively to the Pokémon Owners.

## 4. Termination

This license terminates automatically upon breach of terms or App discontinuation. Licensor may remotely disable access.

## 5. Disclaimers and Limitation of Liability

The App is provided "AS IS" without warranties of any kind, including fitness for purpose or uninterrupted use. Licensor disclaims liability for data loss, device damage, or indirect damages. Direct damages are limited to amounts paid for the App (if any). No accuracy guarantee for card values, rarities, or images sourced under fair use; App does not support real TCG gameplay or pricing.

## 6. Privacy

Use of the App is subject to our Privacy Policy at https://www.pocardex.com/privacy-policy.html, covering data handling per GDPR.

## 7. Updates

Licensor may update this EULA. Continued use constitutes acceptance.

## 8. Governing Law

This EULA is governed by Dutch law. Disputes shall be resolved in courts of Alkmaar, The Netherlands.

Last updated: February 23, 2026.

Contact: privacy@hyperdrivelabs.dev
